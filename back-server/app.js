...
import {
  ...
  createQuestion,
  deleteQuestion,
} from "./questionnaire/question.js";
...
import {
  ...
  updateReco,
  deleteReco,
} from "./questionnaire/recommandation.js";
import {
  updateReponse,
  createReponse,
  deleteSingleReponse,
} from "./questionnaire/reponse.js";
...
app.put("/updateQuestionnaire/:questionnaireId", async (req, res) => {
  ...
  try {
    const questionnaireUpdate = await updateQuestionnaireInfo(
      questionnaireId,
      label,
      description,
      insight,
...
app.post("/createQuestionnaire", async (req, res) => {
  ...
  try {
    const questionnaireCreate = await createQuestionnaire(
      label,
      description,
      insight,
      tooltip,
...
app.put("/updateSection/:sectionId", async (req, res) => {
  ...
  try {
    ...
    const sectionUpdate = await updateSection(sectionId, {
      label,
      description,
      tooltip,
      nbpages,
...
app.put("/questions/:questionId/position", async (req, res) => {
  ...
  try {
    const positionsUpdated = await updatePositions(
      questionId,
      newPosition,
      newPage
    );
...
app.post("/createSection", async (req, res) => {
  ...
  try {
    const SectionCreate = await createSection(
      questionnaire_id,
      label,
      description,
      tooltip,
...
app.post("/createreco", async (req, res) => {
  ...
  try {
    const RecoCreate = await createReco(
      questionnaire_id,
      recommandation,
      min,
      max
...
app.post("/reponses", async (req, res) => {
  const { question_id, label, tooltip, plafond, recommandation, valeurScore } =
    req.body;
...
app.get("/questionnaires/:id/export", async (req, res) => {
  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const question of questions) {
        ...
        if (question.questiontype === "entier") {
          ...
          ["choix_simple", "choix_multiple"].includes(question.questiontype)
        ) {
        ...
        if (dependencies.length > 0) {
          ...
          if (dependencies.length > 1) {
            ...
            questionObj.dependsOn.value = dependencies.map(
              (d) => `r_${d.reponse_id}`
            );
          }
        }
      ...
      if (sectionDeps.length > 0) {
        ...
        if (sectionDeps.length > 1) {
          ...
          sectionObj.dependsOn.value = sectionDeps.map(
            (d) => `r_${d.reponse_id}`
          );
        }
      }
    ...
    const numdiagJson = {
      audit: {
        code: questionnaire.code
          ? questionnaire.code.toString()
          : `AUDIT_${questionnaire.id}`,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="audit_${questionnaire.id}.json"`
    );
...
app.get("/questionnaires/:id/export2", async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const question of questions) {
        ...
        const dependencies = await executeQuery(
          numdiagPool,
          `SELECT qd.reponse_id, r.label as reponse_label, q.id as parent_question_id, q.questiontype as parent_question_type
           FROM QuestionDependencies qd
           JOIN Reponses r ON qd.reponse_id = r.id
           JOIN Questions q ON r.question_id = q.id
           WHERE qd.question_id = $1`,
        ...
        if (dependencies.length > 0) {
          ...
          if (
            parentQuestionType === "choix_multiple" ||
            dependencies.length > 1
          ) {
            operator = "in";
            // Créer un tableau des labels de réponses et le convertir en string JSON
            const valuesArray = dependencies.map((d) => d.reponse_label);
      ...
      const sectionDeps = await executeQuery(
        numdiagPool,
        `SELECT sd.reponse_id, r.label as reponse_label, q.id as parent_question_id, q.questiontype as parent_question_type
         FROM SectionDependencies sd
         JOIN Reponses r ON sd.reponse_id = r.id
         JOIN Questions q ON r.question_id = q.id
         WHERE sd.section_id = $1`,
      ...
      if (sectionDeps.length > 0) {
        ...
        if (parentQuestionType === "choix_multiple" || sectionDeps.length > 1) {
          operator = "in";
          // Créer un tableau des labels de réponses et le convertir en string JSON
          const valuesArray = sectionDeps.map((d) => d.reponse_label);
          value = JSON.stringify(valuesArray);
        } else {
          operator = "equals";
          value = dep.reponse_label;
