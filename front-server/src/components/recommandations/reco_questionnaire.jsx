...
function RecoQuestionnaire({ recommandation, onUpdateReco, onDeleteReco }) {
  ...
  async function updateReco(updates) {
    try {
      const response = await fetch(
        `http://127.0.0.1:3008/updatereco/${recommandation.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          "Erreur lors de la mise à jour en db des infos de recommandation"
        );
      }
  ...
  async function deleteReco() {
    try {
      const response = await fetch(
        `http://127.0.0.1:3008/deletereco/${recommandation.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(
          "Erreur lors de la suppression en db de la recommandation"
        );
      }
      const data = await response.json();
    ...
    <div className="section" style={{ padding: 10 + "px" }}>
      <div className="section-content">
        {buttonUpdateReco === "Modifier" ? (
          ...
          <RecoUpdateForm
            recommandation={isRecommandation}
            onChange={handleInputRecoChange}
          />
        )}
      </div>
