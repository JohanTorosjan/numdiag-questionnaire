...
function Section({
  ...
  const [buttonUpdateSection, setButtonUpdateSection] =
    React.useState("Modifier");
  ...
  const handleSaveQuestion = async (newQuestion) => {
    ...
    const response = await fetch("http://127.0.0.1:3008/questions", {
      method: "POST",
    ...
    if (response.status == 201) {
      ...
      try {
        const responseQ = await fetch(
          `http://127.0.0.1:3008/questionnaire/${questionnaireId}`
        );
        if (!responseQ.ok) {
  ...
  async function updateSection(idSection, updates) {
    try {
      const response = await fetch(
        `http://127.0.0.1:3008/updateSection/${idSection}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          "Erreur lors de la mise à jour en db des infos de section"
        );
      }
  ...
  async function toggleButtonUpdateSection() {
    if (buttonUpdateSection === "Modifier") {
      ...
      try {
        const updateSect = await updateSection(section.id, {
      ...
      <div className="section-header">
        ...
        <div className="section-actions">
          ...
          <button
            ...
            onClick={() =>
              toggleButtonActiveSection(section.id, section.isactive)
            }
          >
