...
function QuestionDependencies({ questionnaire, question, onUpdate }) {
  ...
  const fetchQuestionsAndAnswers = async (questionnaireId) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:3008/questions/${questionnaireId}`
      );
      if (!response.ok) {
      ...
      const filteredQuestions = data
        ...
        .filter((q) => q.questiontype !== "entier");
  ...
  const fetchExistingDependencies = async () => {
    const response = await fetch(
      `http://127.0.0.1:3008/questions/dependencies/${question.id}`
    );
    const data = await response.json();
  ...
  const getSelectedCount = (questionId) => {
    return selectedDependencies.filter((dep) =>
      dep.startsWith(`${questionId}_`)
    ).length;
  };
  ...
  if (error) {
    return (
      <div className="dependencies-error">
        <p>{error}</p>
        <button
          onClick={() => fetchQuestionsAndAnswers(questionnaire)}
    ...
    <div className="question-dependencies">
      {availableQuestions.length === 0 ? (
        <p className="no-questions">
          Aucune autre question disponible dans ce questionnaire.
        </p>
      ) : (
          ...
          {availableQuestions.map((q) => (
            <div key={q.id} className="question-item">
              <div
                ...
                onClick={() =>
                  q.answers.length > 0 && toggleQuestionExpansion(q.id)
                }
              >
                ...
                {q.answers.length > 0 && (
                  <span
                    className={`expand-icon ${
                      expandedQuestions.has(q.id) ? "expanded" : ""
                    }`}
                  >
                    ▼
                  </span>
              ...
              {q.answers.length === 0 ? (
                <p className="no-answers">
                  Cette question n'a pas de réponses prédéfinies (type:{" "}
                  {q.questiontype})
                </p>
              ) : (
