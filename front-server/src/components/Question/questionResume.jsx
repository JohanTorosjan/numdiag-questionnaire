...
function QuestionResume({
  ...
  const handleSaveAnswers = async (newAnswers) => {
    ...
    const response = await fetch("http://127.0.0.1:3008/reponses", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    try {
      ...
      if (
        ...
        const updatePositionResponse = await fetch(
          `http://127.0.0.1:3008/questions/${question.id}/position`,
          {
            method: "PUT",
            headers: {
      ...
      if (question.questiontype !== updatedQuestion.questiontype) {
        const deletingResponse = await fetch(
          `http://127.0.0.1:3008/questions/${question.id}/deleteReponses`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
      ...
      <div className="question-header">
        <div className="question-main-info">
          ...
          <div className="question-badge-info">
            ...
            <span className="question-type-badge">
              {question.public_cible
                ? `Public cible : ${question.public_cible}`
                : "Pas de public"}
            </span>
          </div>
        </div>
